...
export class TradingEngine extends EventEmitter {
  ...
  async inferMigrationFromMcap(mint) {
    ...
    try {
      ...
      if (Number.isFinite(mcap) && mcap > 35000) {
        return false; // migrated (false = migration complete)
      }
      if (Number.isFinite(mcap) && mcap > 0 && mcap <= 35000) {
  ...
  async getRealtimeMcap(mint, forceRefresh = false) {
    ...
    if (this.pumpPortalWs) {
      ...
      if (Number.isFinite(wsMcapUsd) && wsMcapUsd > 0) {
        ...
        if (wsMcapUsd > 35000 && migrationState === null) {
            ...
            if (quoteRes.ok) {
              ...
              if (outAmountLamports > 0) {
                ...
                if (Number.isFinite(mcap) && mcap > 0) {
                  if (mcap > 35000 && migrationState === null) {
                    this.setMigrationState(mint, false, 'mcap-inference');
                  }
  ...
  async handleRealtimeTrade({ mint, payload } = {}) {
    ...
    // Strict mode: do not fallback to non-WS sources for active position updates.
    if (!Number.isFinite(mcap) || mcap <= 0) {
      if (this.pumpPortalWs) {
        const wsMcapUsd = this.pumpPortalWs.getMarketCapUsd(mint);
        if (Number.isFinite(wsMcapUsd) && wsMcapUsd > 0) {
  ...
  async updatePositions(tokens, source = 'poll') {
    for (const token of tokens) {
      ...
      // Print Scan: 35% alpha trail (higher conviction, give room to run)
      // Meme Radar: 35% tight trail (riskier, lock gains quickly)
      // KOL/dual signal modifiers still apply on top.
      // CRITICAL: Trail only activates AFTER PnL has reached the trail threshold.
      //   e.g. 35% trail → peak must hit 1.35x before trail engages.
